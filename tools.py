...
class Tools():
    ...
    def generate_dockerfile(self, image, version, run_command, path, port, build_command=""):
        # Gerar o conteúdo do Dockerfile
        dockerfile_content = f"FROM {image}:{version}\n"
        if build_command:
            dockerfile_content += f"RUN {build_command}\n"
        dockerfile_content += f"EXPOSE {port}\n" if port else ""
        dockerfile_content += f"CMD {run_command}"

        # Gerar o conteúdo do docker-compose.yaml
        compose_yaml_content = f"version: '3'\n\nservices:\n"
        compose_yaml_content += f"  app:\n"
        compose_yaml_content += f"    build:\n"
        compose_yaml_content += f"      context: .\n"
        compose_yaml_content += f"      dockerfile: {path}/Dockerfile\n"
        compose_yaml_content += f"    ports:\n"
        compose_yaml_content += f"      - {port}:{port}\n" if port else ""
        compose_yaml_content += f"    command: {run_command}"

        # Salvar o conteúdo no arquivo Dockerfile
        with open(f"{path}/Dockerfile", "w") as dockerfile:
            dockerfile.write(dockerfile_content)

        # Salvar o conteúdo no arquivo docker-compose.yaml
        with open(f"{path}/docker-compose.yaml", "w") as compose_yaml:
            compose_yaml.write(compose_yaml_content)
    ...
    def bhaskara_resolver(self, a, b, c):
        ...
        if variation_1 > variation_2:
            result = f"{variation_2}; {variation_1}"
